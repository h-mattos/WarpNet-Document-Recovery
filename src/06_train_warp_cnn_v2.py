...
def main():
    deblurred_dir = "./data/deblurred.h5"
    orig_dir = "./data/original.h5"
    ids = list(range(h5py.File(orig_dir, 'r')['imgs'].shape[0]))
    ...
    train_dataset = BlurCNNDataset(blurred_h5_file=deblurred_dir, deblurred_h5_file=orig_dir, ids=train_ids)
    val_dataset = BlurCNNDataset(blurred_h5_file=deblurred_dir, deblurred_h5_file=orig_dir, ids=val_ids)
    # holdout_dataset = BlurCNNDataset(blurred_h5_file=deblurred_dir, deblurred_h5_file=orig_dir, ids=holdout_ids)
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(
                model.state_dict(),
                f"{CHECKPOINT_OUTPUT}/best_dewarp_predictor_v2.pth",
            )
            print(
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(CHECKPOINT_OUTPUT, "metrics_plot_dewarp_v2.png"))
    print(f"Saved metrics plot to {CHECKPOINT_OUTPUT}/metrics_plot_dewarp_v2.png")
